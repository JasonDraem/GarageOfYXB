...
@interface XXYPicScrollView ()<UIScrollViewDelegate>
/* 图片数据源 */
@property (nonatomic,copy) NSArray *imageData;
@end

@implementation XXYPicScrollView{
...
+ (instancetype)picScrollViewWithFrame:(CGRect)frame WithImageUrls:(NSArray<NSString *> *)imageUrl {
    return  [[XXYPicScrollView alloc] initWithFrame:frame WithImageNames:imageUrl];
}
...
- (UIImage *)setImageWithIndex:(NSInteger)index {
    ...
    //从内存缓存中取,如果没有使用占位图片
    UIImage *image = [[[XXYPicScrollViewManager shareManager] webImageCache] valueForKey:_imageData[index]];
...
- (void)getImage {
    ...
    for (NSString *urlSting in _imageData) {
        [[XXYPicScrollViewManager shareManager] downloadImageWithUrlString:urlSting];
    }
}
